...
import { TimeFormat } from '@/services/backend';
...
export const TimeFormatPopup = ({
  ...
  const changeFormat = async (format: TimeFormat) => {
  ...
  return (
    <PopupWindow className={'p-2 text-xs'} onOutsideClick={onOutsideClick} left={left} top={top}>
      <button
        onClick={() => changeFormat(TimeFormat.TwelveHour)}
        className={
      ...
      >
        ...
        {dateType?.timeFormat === TimeFormat.TwelveHour && (
      ...
      <button
        onClick={() => changeFormat(TimeFormat.TwentyFourHour)}
        className={
          'flex w-full cursor-pointer items-center justify-between rounded-lg px-2 py-1.5 hover:bg-main-secondary'
        }
      >
        ...
        {dateType?.timeFormat === TimeFormat.TwentyFourHour && (
          <div className={'ml-8 h-5 w-5 p-1'}>
            <CheckmarkSvg></CheckmarkSvg>
          </div>
        )}
