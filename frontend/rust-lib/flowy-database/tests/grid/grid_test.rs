...
use database_model::entities::{
  CellChangeset, FieldChangesetParams, FieldType, GridBlockInfoChangeset, GridBlockMetaSnapshot,
  RowMetaChangeset, TypeOptionDataFormat,
};
use flowy_database::services::field::{
  DateCellContentChangeset, DateCellData, MultiSelectTypeOptionPB, SelectOption,
  SelectOptionCellContentChangeset, SingleSelectTypeOption, SELECTION_IDS_SEPARATOR,
};
use flowy_database::services::row::{
  decode_cell_data_from_type_option_cell_data, CreateRowMetaBuilder,
};
...
async fn grid_row_add_cells_test() {
  ...
  for field in &test.field_metas {
    match field.field_type {
      FieldType::RichText => {
        builder
          .add_cell(&field.id, "hello world".to_owned())
          .unwrap();
      },
      FieldType::Number => {
      ...
      FieldType::SingleSelect => {
        ...
        builder
          .add_select_option_cell(&field.id, option.id.clone())
          .unwrap();
      },
...
async fn grid_row_add_date_cell_test() {
  ...
  for field in &test.field_metas {
    if field.field_type == FieldType::DateTime {
      ...
      assert!(builder
        .add_cell(&field.id, make_date_cell_string("123"))
        .is_ok());
      assert!(builder
    ...
    for field_meta in field_metas {
      if index == 0 {
        let data = match field_meta.field_type {
          ...
          FieldType::SingleSelect => {
            ...
            SelectOptionCellContentChangeset::from_insert(&type_option.options.first().unwrap().id)
              .to_str()
          },
          FieldType::MultiSelect => {
      ...
      if index == 1 {
        let (data, is_err) = match field_meta.field_type {
          ...
          FieldType::SingleSelect => (
            SelectOptionCellContentChangeset::from_insert("abc").to_str(),
            false,
          ),
          FieldType::MultiSelect => (
            SelectOptionCellContentChangeset::from_insert("abc").to_str(),
