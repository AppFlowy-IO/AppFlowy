...
impl TypeOptionCellDataCompare for ChecklistTypeOption {
  ...
  fn apply_cmp_with_uninitialized(
    ...
    match (cell_data, other_cell_data) {
      (None, Some(cell_data)) if !cell_data.selected_option_ids.is_empty() => Ordering::Greater,
      (Some(cell_data), None) if !cell_data.selected_option_ids.is_empty() => Ordering::Less,
      _ => Ordering::Equal,
    }
  }
}
