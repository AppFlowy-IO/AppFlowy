...
use crate::services::field::{
    ...
    RichTextTypeOptionPB, SingleSelectTypeOptionPB, TypeOption, TypeOptionCellData, TypeOptionTransform,
    URLTypeOptionPB,
};
use bytes::Bytes;
use dashmap::DashMap;
...
where
    T: TypeOption + CellDataDecoder + CellDataChangeset + TypeOptionCellData + TypeOptionTransform + 'static,
{
    pub fn new(inner: T, cell_data_cache: Option<AtomicCellDataCache>) -> Box<dyn TypeOptionCellDataHandler> {
        Box::new(Self { inner, cell_data_cache }) as Box<dyn TypeOptionCellDataHandler>
    }
}
    ...
    T: TypeOption + CellDataDecoder,
{
    fn get_decoded_cell_data(
        ...
        let key = CellDataCacheKeyCal::new(field_rev, decoded_field_type.clone(), &cell_str);
        ...
        if let Some(cell_data_cache) = self.cell_data_cache.as_ref() {
            cell_data_cache.write().insert(&key, cell_data.clone());
        }
        Ok(cell_data)
    }
    ...
    fn set_decoded_cell_data(&self, cell_data: <Self as TypeOption>::CellData, field_rev: &FieldRevision) {
        if let Some(cell_data_cache) = self.cell_data_cache.as_ref() {
            ...
            let key = CellDataCacheKeyCal::new(field_rev, field_type, &cell_str);
            cell_data_cache.write().insert(&key, cell_data);
        }
    }
}
    ...
    T: TypeOption + CellDataDecoder + CellDataChangeset + TypeOptionCellData + TypeOptionTransform,
{
    ...
    fn handle_cell_cmp(&self, left_cell_data: &str, right_cell_data: &str, field_rev: &FieldRevision) -> Ordering {
        let left = self
            .get_decoded_cell_data(left_cell_data.to_owned(), &FieldType::RichText, field_rev)
            .unwrap();
        let right = self
            .get_decoded_cell_data(right_cell_data.to_owned(), &FieldType::RichText, field_rev)
            .unwrap();
        // left.cmp(&right)
        todo!()
    }
...
pub struct TypeOptionCellDataExt<'a> {
    field_rev: &'a FieldRevision,
    cell_data_cache: Option<AtomicCellDataCache>,
}

impl<'a> TypeOptionCellDataExt<'a> {
    ...
    pub fn get_type_option_cell_data_handler(
        ...
        match field_type {
            FieldType::RichText => self
                ...
                .map(|type_option| TypeOptionCellDataHandlerImpl::new(type_option, self.cell_data_cache.clone())),
