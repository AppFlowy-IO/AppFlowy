...
impl UserAuthService for PostgrestUserAuthServiceImpl {
  fn sign_up(&self, params: BoxAny) -> FutureResult<SignUpResponse, FlowyError> {
    ...
    FutureResult::new(async move {
      ...
      Ok(SignUpResponse {
        user_id: user.uid,
        workspace_ids: user.workspace_id,
        ..Default::default()
      })
    })
  }
  ...
  fn sign_in(&self, params: BoxAny) -> FutureResult<SignInResponse, FlowyError> {
    ...
    FutureResult::new(async move {
      let uuid = uuid_from_box_any(params)?;
      let workspace_ids = get_user_workspace_with_uuid(postgrest, uuid)
        .await?
        .into_iter()
        .map(|user_workspace| user_workspace.workspace_id)
        .collect();
      Ok(SignInResponse {
        user_id: user.uid,
        workspace_ids,
  ...
  fn get_user_profile(
    ...
    let postgrest = self.postgrest.clone();
    FutureResult::new(async move {
      let workspace_ids = get_user_workspace_with_uid(postgrest, uid)
        .await?
        .into_iter()
        .map(|user_workspace| user_workspace.workspace_id)
        .collect();
