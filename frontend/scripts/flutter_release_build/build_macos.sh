...
#
# Usage: sudo ./scripts/flutter_release_build/build_macos.sh <build_type> <build_arch> <version> <skip-rebuild>
#
# Options:
...
show_help() {
    echo "Usage: sudo ./scripts/flutter_release_build/build_macos.sh [-h] <build_type> <build_arch> <version> <skip-rebuild>"
    echo ""
    echo "Options:"
...
if [ -z "$BUILD_TYPE" ]; then
    echo "Please specify build type: zip, dmg"
    exit 1
fi
...
if [ -z "$BUILD_ARCH" ]; then
    echo "Please specify build arch: x86_64, arm64 or universal"
    exit 1
fi
