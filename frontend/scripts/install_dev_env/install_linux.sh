...
# Check if the current version is 3.13.0
if [ "$FLUTTER_VERSION" = "3.13.0" ]; then
    echo "Flutter version is already 3.13.0"
else
    ...
    cd $FLUTTER_PATH
    # Use git to checkout version 3.13.0 of Flutter
    git checkout 3.13.0
    # Get back to current working directory
    cd "$current_dir"

    echo "Switched to Flutter version 3.13.0"
fi
