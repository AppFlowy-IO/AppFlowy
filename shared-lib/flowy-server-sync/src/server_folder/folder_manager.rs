...
impl ServerFolderManager {
        ...
        let result = match self.get_folder_handler(&user_id, &folder_id).await {
            None => {
                let _ = self
                    ...
                    .await
                    .map_err(|e| SyncError::internal().context(format!("Server create folder failed: {:?}", e)))?;
                Ok(())
            }
            Some(handler) => {
    ...
    async fn create_folder_handler(&self, folder_info: FolderInfo) -> Result<Arc<OpenFolderHandler>, SyncError> {
...
impl OpenFolderHandler {
    fn new(folder_info: FolderInfo, persistence: Arc<dyn FolderCloudPersistence>) -> SyncResult<Self> {
    ...
    async fn apply_revisions(&self, user: Arc<dyn RevisionUser>, revisions: Vec<Revision>) -> SyncResult<()> {
