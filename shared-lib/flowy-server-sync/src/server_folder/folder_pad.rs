...
impl RevisionSyncObject<EmptyAttributes> for ServerFolder {
    ...
    fn transform(&self, other: &FolderOperations) -> Result<(FolderOperations, FolderOperations), SyncError> {
...
#[inline]
pub fn make_folder_from_revisions(folder_id: &str, revisions: Vec<Revision>) -> Result<Option<FolderInfo>, SyncError> {
