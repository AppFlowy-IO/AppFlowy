use crate::{
  FieldRevision, FieldTypeRevision, FilterRevision, GroupConfigurationRevision, SortRevision,
};
  ...
  T: Debug + Clone + Default + serde::Serialize + serde::de::DeserializeOwned + 'static,
{
  pub fn get_mut_objects(
    ...
    if value.is_none() {
      eprintln!(
        "[Configuration] Can't find the {:?} with",
        std::any::type_name::<T>()
      );
    }
    value
